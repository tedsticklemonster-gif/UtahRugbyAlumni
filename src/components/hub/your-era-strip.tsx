  ...
  return (
    <div className="px-4 pt-4">
      <div className="flex items-end justify-between mb-3">
        <div>
          <span className="block h-[2px] w-6 bg-[#CC0000]" />
          <p className={`${eyebrow} mt-1.5 text-[9px] text-zinc-500`}>Your Era</p>
        ...
        <Link
          ...
          className={`${eyebrow} text-[9px] text-zinc-500 hover:text-white transition-colors`}
        >
          View All →
        </Link>
      </div>
          ...
          >
            {m.photo_signed_url ? (
              <img
                ...
                className="size-12 rounded-full object-cover border-2 border-zinc-800 group-hover:border-[#CC0000] transition-colors"
              />
            ) : (
              <div className="flex size-12 items-center justify-center rounded-full bg-zinc-800 text-xs font-bold text-zinc-400 border-2 border-zinc-800 group-hover:border-[#CC0000] transition-colors">
                {m.first_name[0]}{m.last_name[0]}
              </div>
            )}
            <p className="text-[10px] text-zinc-400 truncate w-full text-center group-hover:text-white transition-colors">
              {m.first_name}
            </p>
          </Link>
        ))}
