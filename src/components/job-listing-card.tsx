...
export function JobListingCard({ job }: { job: JobListing }) {
  ...
  return (
    <div className="surface-card p-5 transition-[border-color,box-shadow] duration-200 ease-out hover:border-border-strong">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <h3 className="text-card-title text-white">{job.title}</h3>
          <p className="flex items-center gap-2 mt-1 text-[0.9375rem] text-zinc-300">
            <Briefcase className="size-4 shrink-0 text-zinc-500" />
            {job.company}
          </p>
        </div>
        {job.alumni_grad_year && (
          <span className="rounded-md bg-utah-red px-2 py-0.5 text-xs font-semibold text-white shrink-0">
      ...
      <p className="text-body-sm mt-3 text-zinc-400 line-clamp-3">
        {job.description}
      </p>
      ...
      <div className="mt-3.5 flex items-center justify-between">
        ...
        {job.apply_url && (
          <a
            ...
            className="inline-flex items-center gap-1.5 rounded-full bg-utah-red px-4 py-2 text-sm font-semibold text-white shadow-card hover:bg-utah-red/90 transition-colors"
          >
